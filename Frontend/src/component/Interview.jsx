...
const calculateEAR = (landmarks) => {
  ...
  const rightEAR =
    (getDistance(rightEye.p2, rightEye.p6) +
      getDistance(rightEye.p3, rightEye.p5)) /
    (2 * getDistance(rightEye.p1, rightEye.p4));
  const leftEAR =
    (getDistance(leftEye.p2, leftEye.p6) +
      getDistance(leftEye.p3, leftEye.p5)) /
    (2 * getDistance(leftEye.p1, leftEye.p4));
...
export default function Interview() {
  ...
  const handleStartInterview = async () => {
    ...
    try {
      // 👇 api se backend call
      const res = await api.post('/api/interview/start', { candidateName });
      ...
      if (faces && faces.length === 1) {
        ...
        if (ear < EAR_THRESHOLD) {
          if (!drowsinessTimer.current)
            drowsinessTimer.current = setTimeout(() => {
              flagEvent('DROWSINESS_DETECTED (3s)');
              drowsinessTimer.current = null;
            }, 3000);
        } else {
          if (drowsinessTimer.current) clearTimeout(drowsinessTimer.current);
          drowsinessTimer.current = null;
        }
      } else {
        if (lookingAwayTimer.current) clearTimeout(lookingAwayTimer.current);
  ...
  useEffect(() => {
    if (faceMeshModel) faceMeshModel.onResults(onFaceMeshResults);
  }, [faceMeshModel, onFaceMeshResults]);
  ...
  const handleObjectDetections = (predictions) => {
    ...
    if (detected.has('laptop') || detected.has('tv'))
      flagEvent('EXTRA_DEVICE_DETECTED');
  };
  ...
  useEffect(() => {
    ...
    const objInterval = setInterval(async () => {
      if (
        ...
        !isDetectingObjects.current
      ) {
        isDetectingObjects.current = true;
        try {
          const pred = await objectModel.detect(videoRef.current, 20, 0.4);
          handleObjectDetections(pred);
        } catch (err) {
          console.error(err);
        } finally {
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#0b1220] px-6">
        ...
        <div className="w-full max-w-md bg-gray-900 p-10 rounded-3xl shadow-2xl border border-gray-700">
          ...
          <p className="text-gray-400 text-center mb-8">
            Enter your details below to begin the interview
          </p>
  ...
  return (
    <div className="min-h-screen bg-[#0b1220] text-white flex flex-col items-center px-6 py-8">
      ...
      <div className="mb-4 px-6 py-3 bg-gradient-to-r from-indigo-600 to-blue-500 rounded-full shadow-lg text-lg font-semibold text-white">
      ...
      <div className="w-full max-w-3xl bg-[#071127] rounded-xl shadow-2xl overflow-hidden relative border border-gray-800 mb-6">
        ...
        />
        <div className="absolute top-4 right-4 bg-black/50 px-3 py-1 rounded-full text-sm font-medium border border-gray-700">
          {status}
        </div>
      </div>
      <div className="flex items-center gap-4 mb-6">
        <button
          ...
          className="px-6 py-3 rounded-lg bg-red-600 hover:bg-red-700 font-bold shadow-md"
        >
          Stop Interview & Save
        </button>
        <div className="text-sm text-gray-400">
          Models: {faceMeshModel ? 'FaceMesh ✓' : 'FaceMesh —'} •{' '}
      ...
      <div className="w-full max-w-3xl grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-5 bg-[#071127] rounded-xl shadow-lg border border-gray-800">
          ...
          <p className="mt-3 text-sm text-gray-400">
            Realtime monitoring: face, gaze, drowsiness, objects, audio
          </p>
        </div>
        <div className="p-5 bg-[#071127] rounded-xl shadow-lg border border-gray-800 h-48 overflow-y-auto">
          ...
          <ul className="space-y-1 text-yellow-300 text-sm">
            {events.length === 0 ? (
              <li className="text-gray-500 italic">No events detected yet</li>
            ) : (
              events.map((e, i) => (
